...
#else
	/* data direction for out */
	mmio_clrsetbits_32(0x27014000 + 0x4, 1 << 5, 1 << 5);
	/* gpio4 pinnum:5 */
	mmio_clrsetbits_32(0x27014000 + 0x0, 1 << 5, 1 << 5);
#endif
	udelay(1000);
	/* set ms to 1 */
...
#else
	/* data direction for out */
	mmio_clrsetbits_32(0x27014000 + 0x4, 1 << 6, 1 << 6);
	/* gpio4 pinnum:6 */
	mmio_clrsetbits_32(0x27014000 + 0x0, 1 << 6, 1 << 6);
#endif
	udelay(1000);
	/* set ms to 1 */
...
static int cvi_sdhci_probe(struct udevice *dev)
{
	...
	if (host->index == MMC_TYPE_MMC) {
		...
		mmio_clrsetbits_32(0x27014000 + 0x4, 1 << 5, 1 << 5);
		/* set to gpio to 0 */
		mmio_clrsetbits_32(0x27014000 + 0x0, 1 << 5, 0);
#endif
		udelay(1000);
		/* set ms to 3.3v */
		mmio_clrsetbits_32(0x281051e0, 1 << 6, 0);
		...
		mmio_clrsetbits_32(0x27014000 + 0x4, 1 << 6, 1 << 6);
		/* set to gpio to 0 */
		mmio_clrsetbits_32(0x27014000 + 0x0, 1 << 6, 0);
#endif
		udelay(1000);
		/* set ms to 3.3v */
		mmio_clrsetbits_32(0x281051e0, 1 << 4, 0);
...
static const struct udevice_id cvi_sdhci_match[] = {
	{
		.compatible = "cvitek,athena2-emmc",
		.data = (ulong)&sdhci_cvi_emmc_drvdata,
	},
	{
		.compatible = "cvitek,athena2-sd",
		.data = (ulong)&sdhci_cvi_sd_drvdata,
	},
	{
		.compatible = "cvitek,athena2-sd1",
		.data = (ulong)&sdhci_cvi_sd1_drvdata,
	},
	{
		.compatible = "cvitek,athena2-sdio",
