...
static void io_config(int channel)
{
	switch (channel) {
	case ADC1:
		PINMUX_CONFIG(ADC1, ADC1, G12);
		mmio_clrbits_32(REG_ADC1, ADC_IE | PD_EN);
		break;
	case ADC2:
		PINMUX_CONFIG(ADC2, ADC2, G12);
		mmio_clrbits_32(REG_ADC2, ADC_IE | PD_EN);
		break;
	case ADC3:
		PINMUX_CONFIG(ADC3, ADC3, G12);
		mmio_clrbits_32(REG_ADC3, ADC_IE | PD_EN);
		break;
	case PWR_ADC1:
		PINMUX_CONFIG(PWR_GPIO0, PWR_SAR0, G7);
		mmio_clrbits_32(REG_ADC4, ADC_IE | PD_EN);
		break;
	case PWR_ADC2:
		PINMUX_CONFIG(PWR_GPIO1, PWR_SAR1, G7);
		mmio_clrbits_32(REG_ADC5, ADC_IE | PD_EN);
		break;
	default:
