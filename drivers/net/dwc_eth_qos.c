...
{
#if IS_ENABLED(CONFIG_TARGET_CVITEK_ATHENA2)
...
static int eqos_mdio_register(struct udevice *dev)
{
	...
	if (ofnode_valid(fl_node)) {
		...
		if (!phy_detected) {
			printf("No phy could be detected\n");
			return -ENOLINK;
		}
	}
	...
	if (ret < 0) {
		printf("mdio_register() failed: %d", ret);
		goto err_free_mdio;
	}
	...
	ret = phy_config(eqos->phy);
	if (ret < 0) {
		printf("phy_config() failed: %d", ret);
		goto err_free_mdio;
	}
...
static int eqos_reset_phy(struct udevice *dev)
{
	...
	if (!eqos->has_phy_reset_gpio) {
		printf("no phy_reset_gpio, reset phy failed");
		return -1;
	}

	ret = dm_gpio_set_value(&eqos->phy_reset_gpio, 0);
	if (ret < 0) {
		printf("dm_gpio_set_value(phy_reset, assert) failed: %d", ret);
		return ret;
	}
	...
	if (ret < 0) {
		printf("dm_gpio_set_value(phy_reset, deassert) failed: %d", ret);
		return ret;
	}
...
#if defined(CONFIG_TARGET_CVITEK_ATHENA2)
