...
#ifdef CONFIG_TARGET_CVITEK_ATHENA2
	int dma_spi_nand = 0x3e;
#else
	int dma_spi_nand = 0x26;
#endif
	if (DEBUG_DMA)
...
static int nand_adjust_param(struct cvsnfc_host *host)
{
	u32 val = 0;
	void __iomem *g11_pinmux_addr = (void __iomem *)0x28104b00;

	printf("[%s %d] \n", __func__, __LINE__);
	val = readl(host->regbase + 0x04);

	// if (val&0xff0000) {
	// 	val = (1<<31) | (0x0<<16) | (0x100);
	// } else {
		val = (0<<31) | (0x37<<16) | (0x100);
	// }
	writel(val, host->regbase + 0x24);
	// set pin drv
	writel(0x2524, g11_pinmux_addr + 0x1c);
	writel(0x2f24, g11_pinmux_addr + 0x20);
	writel(0x2f24, g11_pinmux_addr + 0x24);
	writel(0x2f24, g11_pinmux_addr + 0x28);
	writel(0x2f24, g11_pinmux_addr + 0x2c);
	writel(0x2f24, g11_pinmux_addr + 0x30);
...
#ifdef CONFIG_TARGET_CVITEK_ATHENA2
	host->dmabase = (void __iomem *)0x29340000;
#else
