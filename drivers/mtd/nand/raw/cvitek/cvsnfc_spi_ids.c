...
#define SAMPLE_PARAM (BIT_REG_RSP_DLY_OW_EN | BIT_REG_DLY_SEL(0x3c) | BIT_REG_RSP_DLY_OW_VAL)
...
struct cvsnfc_chip_info cvsnfc_spi_nand_flash_table[] = {
	...
	/* 25N02KWZEIR 1.8V */
	{
		...
		.badblock_pos = BBP_FIRST_PAGE,
		.driver    = &spi_nand_driver_esmt,
		.flags = FLAGS_ENABLE_X2_BIT | FLAGS_ENABLE_X4_BIT | FLAGS_SET_PLANE_BIT,
		.ecc_en_feature_offset = 0xb0, /* Configuration register */
		.ecc_en_mask = 1 << 4, /* bit 4 */
		.ecc_status_offset = 0xc0, /* Status register */
		.ecc_status_mask = 0x30, /* bit 4 & 5 */
...
void nand_ip_init(struct cvsnfc_host *host, struct cvsnfc_chip_info *spi_dev)
{
	u32 trx_ctrl1 = 0;

	trx_ctrl1 = cvsfc_read(host, REG_SPI_NAND_TRX_CTRL1) & (~BIT_REG_NAND_SCK_MASK);
	trx_ctrl1 |= SPI_NAND_SET_SCK_L(spi_dev->sck_l) | SPI_NAND_SET_SCK_H(spi_dev->sck_h);
	cvsfc_write(host, REG_SPI_NAND_TRX_CTRL1, trx_ctrl1);
	cvsfc_write(host, REG_SPI_NAND_BOOT_CTRL, spi_dev->sample_param);
}
