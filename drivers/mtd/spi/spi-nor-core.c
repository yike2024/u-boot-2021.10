...
static void gd25lq128_post_sfdp_fixup(struct spi_nor *nor,
                                    struct spi_nor_flash_parameter *params)
{
        params->quad_enable = spansion_read_cr_quad_enable;
}

static struct spi_nor_fixups gd25lq128_fixups = {
        .post_sfdp = gd25lq128_post_sfdp_fixup,
};

static int none_quad_enable(struct spi_nor *nor)
{
	return 0;
}

static void gd25lb512_post_sfdp_fixup(struct spi_nor *nor,
                                    struct spi_nor_flash_parameter *params)
{
        params->quad_enable = none_quad_enable;
}

static struct spi_nor_fixups gd25lb512_fixups = {
        .post_sfdp = gd25lb512_post_sfdp_fixup,
};
...
void spi_nor_set_fixups(struct spi_nor *nor)
{
	...
	if (!strcmp(nor->info->name, "GD25LQ128E"))
		nor->fixups = &gd25lq128_fixups;

	if (!strcmp(nor->info->name, "GD25LB512") || !strcmp(nor->info->name, "GD25LB256E"))
		nor->fixups = &gd25lb512_fixups;
