...
int usb_add_gadget_udc_release(struct usb_gadget *gadget)
{
	...
	mutex_lock(&udc_lock);
	list_add_tail(&g_udc.list, &udc_list);
...
{
	pr_debug("unregistering UDC driver [%s]\n",	udc->driver->function);
...
void usb_del_gadget_udc(struct usb_gadget *gadget)
{
	...
	ERROR("gadget not registered.\n");
	mutex_unlock(&udc_lock);

	return;

found:
	VERBOSE("unregistering gadget\n");
...
static int udc_bind_to_driver(struct usb_udc *udc, struct usb_gadget_driver *driver)
{
	...
	pr_debug("registering UDC driver [%s]\n", driver->function);
