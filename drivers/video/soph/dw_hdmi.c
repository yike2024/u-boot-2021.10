...
static void hdmi_writeb(struct dw_hdmi *hdmi, u8 val, int offset)
{
	_reg_write((uintptr_t)(hdmi->regs + (offset << 2)), val);
}

static u8 hdmi_readb(struct dw_hdmi *hdmi, int offset)
{
...
static int soph_dw_hdmi_scrambling_enable(struct dw_hdmi *hdmi,
					      int enable, bool set)
{
	u8 stat;
	int ret;

	ret = drm_scdc_readb(&hdmi->adap, SCDC_TMDS_CONFIG, &stat);
	if (ret < 0) {
	...
	if (enable == 1) {
		...
		if (set)
			...
			stat |= SCDC_TMDS_BIT_CLOCK_RATIO_BY_10;
...
static void soph_dw_hdmi_scdc_set_tmds_rate(struct dw_hdmi *hdmi)
{
	u8 stat;
...
static int hdmi_phy_configure(struct dw_hdmi *hdmi)
{
	...
	if (hdmi->edid_data.display_info.hdmi.scdc.supported)
		soph_dw_hdmi_scdc_set_tmds_rate(hdmi);
...
static int dw_hdmi_phy_init(struct soph_connector *conn, struct dw_hdmi *hdmi,
			    void *data)
{
	int i, ret;
...
static void dw_hdmi_phy_disable(struct soph_connector *conn, struct dw_hdmi *hdmi,
				void *data)
{
	dw_hdmi_phy_power_off(hdmi);
}
			     ...
			     const struct drm_display_mode *mode)
{
	...
	if (hdmi_info->scdc.supported) {
		if (vmode->mtmdsclock > 340000000 ||
			...
			soph_dw_hdmi_scrambling_enable(hdmi, 0, false);
		}
	}
...
static void dw_hdmi_disable(struct soph_connector *conn, struct dw_hdmi *hdmi,
...
static void hdmi_tx_hdcp_config(struct dw_hdmi *hdmi,
				const struct drm_display_mode *mode)
{
	u8 vsync_pol, hsync_pol, data_pol, hdmi_dvi, de;
			 ...
			 struct display_state *state)
{
	...
	if (!hdmi->vic)
		printf("Non-CEA mode used in HDMI %dx%d@%d\n",
...
static void initialize_hdmi_mutes(struct dw_hdmi *hdmi)
{
	...
	/*Force output black*/
	hdmi_writeb(hdmi, 0x00, HDMI_FC_DBGTMDS2);
	hdmi_writeb(hdmi, 0x00, HDMI_FC_DBGTMDS1);
	hdmi_writeb(hdmi, 0x00, HDMI_FC_DBGTMDS0);
}
...
int soph_dw_hdmi_init(struct soph_connector *conn, struct display_state *state)
{
	...
	/*
	 * use the default timing scl clock rate is about 99.6KHz.
	 */
