...
int check_and_update_fip_bin(void)
{
	...
	// boot_src = readl((unsigned int *)BOOT_SOURCE_FLAG_ADDR);		// debug cv186x
...
retry:
			...
			for (u32 pos = 0; pos < pg_sz; pos++) {
				if (*(temp_buf+pos) != *(pg_buf+pos)) {
					printf("fip read back compare error, %d: write: 0x%x, read: 0x%x\n",
						 pos, *(temp_buf+pos), *(pg_buf+pos));
					// cvi_assert(0);
				}
			}
		}
